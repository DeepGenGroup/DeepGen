...
@kcg_kernel
def _attn_split_k1_kernel(q_ptr, k_ptr, em_ptr, denom_ptr):
    'DUMP CODES'
    pass


@kcg_kernel
def _attn_split_k2_kernel(q_ptr, k_ptr, v_ptr, em_ptr, denom_ptr, out_ptr):
    'DUMP CODES'
    pass


def _attn_split_k1(q: torch.Tensor, k: torch.Tensor,
                    em: torch.Tensor, denom: torch.Tensor):
    return _attn_split_k1_kernel(q, k, em, denom)


def _attn_split_k2(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
                    em: torch.Tensor, denom: torch.Tensor, out: torch.Tensor):
    return _attn_split_k2_kernel(q, k, v, em, denom, out)
...
def _precompute_em_denom(q, k, device, dtype):
    """Compute standard attention em/denom via PyTorch for standalone K2 tuning."""
...
class AttentionSplitOp(OpInterface):
    """Split attention into two codegen'd kernels:

      Kernel 1 (stats):  GEMM(Q@K^T) + online reduce -> em, denom
        Signature: (Q_t[B,H,D,S], K[B,H,D,S], em_out[B,H,S,1], denom_out[B,H,S,1])
        C++ entry: compile_attn_split_k1

      Kernel 2 (output): GEMM(Q@K^T) + normalize(em,denom) + GEMM(P@V) -> O
        Signature: (Q_t[B,H,D,S], K[B,H,D,S], V[B,H,S,D], em[B,H,S,1], denom[B,H,S,1], O[B,H,S,D])
        C++ entry: compile_attn_split_k2

    Benchmark runs kernel1 then kernel2, timing their sum.
    Baseline is standard attention for comparison.
    """
    ...
    def _dump_tensor_stats(self, name: str, t: torch.Tensor):
        ...
        print(f"[attn-split-debug] {name}: shape={tuple(t.shape)} min={t_min} max={t_max} "
    ...
    def InitLibInterface(self):
        if self.CompileKernelK2 is None or self.SetPlatform is None:
            ...
            self.CompileKernelK1 = getattr(mod, 'compile_attn_split_k1', None)
            self.CompileKernelK2 = getattr(mod, 'compile_attn_split_k2', None)
            if self.CompileKernelK1 is None:
                print("[attn_split] WARNING: compile_attn_split_k1 not found in libdeepgen", flush=True)
            if self.CompileKernelK2 is None:
                print("[attn_split] WARNING: compile_attn_split_k2 not found in libdeepgen", flush=True)
    ...
    def Compile(self, deviceId, backendtype, arch, info, opt=None):
        ...
        if self.CompileKernelK1 is None or self.CompileKernelK2 is None:
            raise RuntimeError("[attn_split] compile_attn_split_k1/k2 not available in libdeepgen.")

        # Compile kernel 1 (stats)
    ...
    def _get_k1_signature(self, dtype):
        ...
        return _attn_split_k1(q, k, em, denom)
    ...
    def GetSignature(self, dtypes):
        ...
        return _attn_split_k2(q, k, v, em, denom, out)
...
class _AttentionSplitSingleKernelBase(OpInterface):
    """Shared base for AttentionK1Op / AttentionK2Op."""
...
class AttentionK1Op(_AttentionSplitSingleKernelBase):
    """Tune split attention kernel 1 (GemmStats) independently."""

    def InitLibInterface(self):
        mod = self._init_lib()
        if mod:
            self._compile_func = getattr(mod, "compile_attn_split_k1")
...
class AttentionK2Op(_AttentionSplitSingleKernelBase):
    """Tune split attention kernel 2 (FlashAttnSplitK2) independently."""

    def InitLibInterface(self):
        mod = self._init_lib()
        if mod:
            self._compile_func = getattr(mod, "compile_attn_split_k2")
