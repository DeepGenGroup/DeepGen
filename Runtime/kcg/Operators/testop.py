...
def _get_sig(a : torch.Tensor, out : torch.Tensor ):
    dimsizeA = len(a.shape) 
    dimsizeD = len(out.shape)
    assert dimsizeA == dimsizeD, "In out must with same dim size"
    return _kernel_reduce(
        a, out
    )
...
##########        
    ...
    def GetSignature(self, dtypes : List[torch.dtype]) -> dict :
        ...
        a = torch.randn((1,3,100,100), device='cpu', dtype=dtypeA)
        # get function signature
        def _get_broadcast_signature(a : torch.Tensor ):
            return _kernel_broadcast(
                a
            )
        outSignature = _get_broadcast_signature(a)
        return outSignature
...
##########        
    ...
    def GetSignature(self, dtypes : List[torch.dtype]) -> dict :
        ...
        a = torch.randn((1,3,100,100), device='cpu', dtype=dtypeA)
        out = torch.randn((1,3,100,100), device='cpu', dtype=dtypeA)
        # get function signature
        outSignature = _get_sig(a,out)
        return outSignature
...
class DebugAttnOp(OpInterface) :
    ...
    def GetBaselineInputTensor(self, devId : int) -> List[torch.Tensor] : 
        if self.InputTensors_Baseline is None :
            ...
            ety = ToTorchType(EnumKernelDType(dtypeInt))
            # print("ety =", ety)
            q = torch.rand((b0, b1, m,n),dtype=ety, device=f"cuda:{devId}" )  # matmul(softmax(matmul(mn, nm)) , mn) = mn
            k = torch.rand((b0, b1, m,n),dtype=ety, device=f"cuda:{devId}" )  # matmul(softmax(matmul(mn, nm)) , mn) = mn
            v = torch.rand((b0, b1, m,n),dtype=ety, device=f"cuda:{devId}" )  # matmul(softmax(matmul(mn, nm)) , mn) = mn
...
##########        
    ...
    def GetSignature(self, dtypes : List[torch.dtype]) -> dict :
        ...
        o = torch.randn((1,3,100,100), device='cpu', dtype=dtypeA)
        p = torch.randn((1,3,100,100), device='cpu', dtype=dtypeA)
        # get function signature
        def _getsig(q,k,v,o,p : torch.Tensor ):
            return _kernel_debugAttn(
                q,k,v,o,p
            )
        outSignature = _getsig(q,k,v,o,p)
        return outSignature
...
##########        
    ...
    def GetSignature(self, dtypes : List[torch.dtype]) -> dict :
        ...
        q = torch.randn((100,100), device='cpu', dtype=dtypeA)
        k = torch.randn((100,100), device='cpu', dtype=dtypeA)
        # get function signature
        outSignature = _kernel_reduce(q,k)
        return outSignature
