...
class FeedForward(nn.Module):
    def __init__(self, dim: int, hidden_dim: int, multiple_of: int, ffn_dim_multiplier: Optional[float], base="our"):
    ...
    def forward(self, x):
        return self.w2(F.silu(self.w1(x).to(self.device)).to("mlu") * self.w3(x))  # batch*2048*4096 <dot> batch*4096*11008 -> x2  / batch*2048*11008 <dot> batch*11008*4096
        # return F.silu(x) * x


class TransformerBlock(nn.Module):
    def __init__(self, layer_id: int, args: ModelArgs, base="our"):
        super().__init__()
        self.n_heads = args.n_heads
        self.dim = args.dim
        self.head_dim = args.dim // args.n_heads
...
class LLAMA2(nn.Module):
    def __init__(self, base="our"):
        ...
        for layer_id in range(args.n_layers):
            self.layers.append(TransformerBlock(layer_id, args, base))
