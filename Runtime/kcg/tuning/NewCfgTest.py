...
class CreateMatmulConfig:
        ...
        for pshread in self.cfg_dict[kw.KEY_SHARED_PREFETCH]:
          for preg in self.cfg_dict[kw.KEY_REG_PREFETCH]:
            ...
            if preg == 1:   # double registers buf
              reg_size = reg_size * 2
            if reg_size * self.type_width <= self.max_reg_size and sm_size * self.type_width <= self.max_sm_size:  # reg & shared memory limit
  ...
  def ultimate(self, space):
    def iscut(thread_size, block_scatter, warp_scatter, warp_layout, type_width):
      # is cut => ture
      rep = (thread_size / block_scatter) * (block_scatter / warp_scatter)
      best_rep = warp_layout * thread_size * (4 // type_width) // 32
      best_rep = 1 if best_rep == 0 else best_rep
      return rep != best_rep
    ...
    for case in space:
      ...
      # if bm <= 128 or bn <= 128:
        ...
        if load_width_per_thread_a >= 4 and load_width_per_thread_b >= 4:
          if not iscut(tm, case[5][0], case[6][0], case[4][0], self.type_width) and not iscut(tn, case[5][1], case[6][1], case[4][1], self.type_width):
