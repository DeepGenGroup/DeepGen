...
__global__ void exp_test(float *A) {
  ...
  for (int bx=0; bx<N; bx+=BN) {
    ...
    for (int i=0; i<TM; i++) {
      for (int j=0; j<TN; j++) {
          ...
          // float newMax = rowMax[i] * tile[i * TN + j];
          rowSum[i] = rowSum[i] * exp(rowMax[i] - newMax) + exp(tile[i * TN + j] - newMax);
          // rowSum[i] = rowSum[i] * (rowMax[i] - newMax) + (tile[i * TN + j] - newMax);
          rowMax[i] = newMax;
      }
    }
    for (int i=0; i<TM; i++) {
      for (int j=0; j<TN; j++) {
        ...
        // tile[i * TN + j] = (tile[i * TN + j] - rowMax[i]) / rowSum[i];
      }
    }
    // store
    for (int i=0; i<TM; i++) {
...
void launch_hsaco(const char *path, float *h, float *d, int len) {
  ...
  hipError_t error = hipModuleGetFunction(&kernel, module, "exp_test");
  if (error == hipSuccess) {
