#!/bin/bash
# === [3/4] Tuning K3 (FlashAttnSplitK2) === 时 libdeepgen 崩溃. 需排查
set -e
...
date
python SimpleLocalTester.py ${CFG} ./testattn_h2o_k1_${SEQLEN}.json 0 10 0 0 h2o_k1 ${DEV} ${DTYPE} --seqlen "$SEQLEN"

echo ""
echo "=== [2/4] Tuning K2 (GemmNormColSum) ==="
date
python SimpleLocalTester.py ${CFG} ./testattn_h2o_k2_${SEQLEN}.json 0 10 0 0 h2o_k2 ${DEV} ${DTYPE} --seqlen "$SEQLEN"

echo ""
echo "=== [3/4] Tuning K3 (FlashAttnSplitK2) ==="
date
python SimpleLocalTester.py ${CFG} ./testattn_h2o_k3_${SEQLEN}.json 0 10 0 0 h2o_k3 ${DEV} ${DTYPE} --seqlen "$SEQLEN"
