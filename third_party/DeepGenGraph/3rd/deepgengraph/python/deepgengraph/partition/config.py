...
from deepgengraph.transform.common import module_to_py, kernel_to_py
...
class PartitionConfig:
  ...
  def optimize(self):
    from deepgengraph.transform.common import optimize
  ...
  def _add_uncovered_ops(self):
    ...
    if len(all_op_ids) > 0:
      for i, op_id in enumerate(all_op_ids):
        ...
        # FIXME: now it should be only 'deepgengraph.avg_pool'
        assert op.get_name() == "deepgengraph.avg_pool", f"{op.get_name()=}"
